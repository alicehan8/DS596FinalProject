import wandb
import torch
import os
from torch.optim import  AdamW
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
import argparse
from einops import rearrange
import sys
from einops.layers.torch import Rearrange
# from sei import *
# from pretrain_multihead import *
from Pretrain_Moe import *
# from DeepHistone import NetDeepHistone
from utils import *
import pandas as pd
from torch.utils.data import Dataset, DataLoader
from transformers import AutoModelForMaskedLM, AutoTokenizer
from transformers import AutoConfig, AutoModelForMaskedLM
from torcheval.metrics import BinaryAccuracy, BinaryAUROC, BinaryF1Score, BinaryAUPRC

import torch.distributed as dist
from torch.nn.parallel import DistributedDataParallel as DDP
from torch.utils.data.distributed import DistributedSampler
from torch.cuda.amp import autocast, GradScaler

        
def main(args):

    dist.init_process_group(backend='nccl')
    local_rank = int(os.environ["LOCAL_RANK"])
    torch.cuda.set_device(local_rank)
    device = torch.device("cuda", local_rank)

    print(f"Rank {dist.get_rank()} using GPU {torch.cuda.current_device()}")


    # histone = "H3K27me3"
    # histone = "H3K27ac"
    histone = "H3K4me3"
    model_name = "LLM_Moe_aml"
    # model_name = "CNN_Moe"
    print("load parameters")
    model_path = "./models/%s_%s.pt" %(histone,model_name)
    early_stop = 0
    max_early_stop = 5
    batch_size = 8 
    best_loss = 100
    seq_length = 2048 #4096
    # seq_length = 5120
    # task_dict = {"task1":6,"task2":5,"task3":4,"task4":4,"task5":3}
    task_dict = {"aml": 1}


    print("load datasets...")
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if model_name == "CNN_Moe":
        data_module = AMLDataModule("./Dataset/%s_AML_generated.csv" %histone,["chr10"], ["chr8","chr9"],seq_length, batch_size,pretrain=False)
    else:
        data_module = AMLDataModule("./Dataset/%s_AML_generated.csv" %histone,["chr10"], ["chr8","chr9"],seq_length, batch_size,pretrain=True)
    # train_loader = data_module.train_dataloader()

    train_dataset = data_module.train_data
    train_sampler = DistributedSampler(train_dataset)
    train_loader = DataLoader(
        train_dataset,
        batch_size=batch_size,
        sampler=train_sampler,
        num_workers=6
    )

    # vali_loader = data_module.val_dataloader()
    val_sampler = DistributedSampler(data_module.vali_data, shuffle=False)
    vali_loader = DataLoader(
        data_module.vali_data,
        batch_size=batch_size,
        sampler=val_sampler,
        num_workers=6
    )

    # test_loader = data_module.test_dataloader()    
    test_sampler = DistributedSampler(data_module.test_data)
    test_loader = DataLoader(
        data_module.test_data,
        batch_size=batch_size,
        sampler=test_sampler,
        num_workers=6
    )

    print("Steps per epoch:", len(train_loader))


    # model = Sei(n_genomic_features = 22).to(device)
    # model = PretrainModel().to(device)
    # model = Pretrain_BLSTM().to(device)
    # model = NetDeepHistone().to(device)
    if model_name == "CNN_Moe":
        model = CNN_Moe(task_dict).to(device)
        model = DDP(model, device_ids=[local_rank], output_device=local_rank)
    else:
        model = Pretrain_Moe(task_dict).to(device)
        # model.model.gradient_checkpointing_enable()
        # model.model.config.use_cache = False
        model = DDP(model, device_ids=[local_rank], output_device=local_rank)
    AWL = AutomaticWeightedLoss(5)
    AWL.to(device)
    # param_groups = param_groups_lrd(model,AWL=AWL)
    # optim = AdamW([{'params': model.parameters()},
    #             {'params': AWL.parameters()}], lr=5e-5)
    optim = AdamW(model.parameters(), lr=5e-5)
    scaler = GradScaler()

    if args.reload:
        checkpoint = torch.load(model_path)
        model.load_state_dict(checkpoint['model_state_dict'])
        optim.load_state_dict(checkpoint['optimizer_state_dict'])

    if dist.get_rank() == 0 and args.wandb_report:
        if args.reload:
            wandb.init(project="AD", resume="must", id=args.wandbId)
            print("wandb resumed")
        else:
            os.environ["WANDB_PROJECT"] = "AD"
            print("init wandb")
            wandb.init(
                project='AD',
                config={
                    "architecture": model_name,
                    "saved_model":model_path,
                    "batch_size":batch_size,
                    "structure": str(model)
                }
            )

    def evaluate_model(model, data_loader, device):
        model.eval()

        # --- Loss tracking (as tensors for all_reduce) ---
        loss_sum = torch.tensor(0.0, device=device)
        loss_count = torch.tensor(0.0, device=device)

        # --- Store local predictions ---
        all_logits = []
        all_labels = []

        with torch.no_grad():
            for batch in data_loader:
                data = batch['sequence'].to(device)
                labels = batch['label'].to(device)

                # outputs, aux_loss = model(data)

                # Concatenate task outputs
                # outputs = torch.cat([v for v in outputs.values()], dim=1)

                outputs, aux_loss = model(data)

                if isinstance(outputs, dict):
                    outputs = torch.cat([v for v in outputs.values()], dim=1)

                outputs = outputs.mean(dim=1, keepdim=True)

                loss = torch.nn.functional.binary_cross_entropy_with_logits(outputs, labels)

                # Accumulate loss properly
                loss_sum += loss.detach()
                loss_count += 1

                # Store predictions
                logits = torch.sigmoid(outputs)
                all_logits.append(logits.detach().cpu())
                all_labels.append(labels.detach().cpu())

        # --- Aggregate loss across GPUs ---
        if torch.distributed.is_initialized():
            torch.distributed.all_reduce(loss_sum, op=torch.distributed.ReduceOp.SUM)
            torch.distributed.all_reduce(loss_count, op=torch.distributed.ReduceOp.SUM)

        mean_loss = (loss_sum / loss_count).item()

        # --- Gather predictions across GPUs safely ---
        local_logits = torch.cat(all_logits, dim=0)
        local_labels = torch.cat(all_labels, dim=0)

        if torch.distributed.is_initialized():
            world_size = torch.distributed.get_world_size()

            gathered_logits = [None for _ in range(world_size)]
            gathered_labels = [None for _ in range(world_size)]

            torch.distributed.all_gather_object(gathered_logits, local_logits)
            torch.distributed.all_gather_object(gathered_labels, local_labels)

            if torch.distributed.get_rank() == 0:
                all_logits_cat = torch.cat(gathered_logits, dim=0).squeeze()
                all_labels_cat = torch.cat(gathered_labels, dim=0).squeeze()

                metries = multiperformance()
                metries.update(all_logits_cat, all_labels_cat)
                # metries.update(local_logits.squeeze(), local_labels.squeeze())
                metrics = metries.compute()
            else:
                metrics = None
        else:
            # Single GPU fallback
            metries = multiperformance()
            # metries.update(local_logits, local_labels)
            metries.update(local_logits.squeeze(), local_labels.squeeze())
            metrics = metries.compute()

        return mean_loss, metrics

    print("training....")

    global_step = 0
    
    for epoch in range(args.epochs):
        if dist.is_initialized():
            train_loader.sampler.set_epoch(epoch)
        model.train()
        AWL.train(True)
        loss_all = {"loss_sum":0,"loss_count":0}
        print("epoch"+str(epoch))
        for batch in train_loader:
            optim.zero_grad()
            data, labels = batch['sequence'].to(device),batch['label'].to(device)
            # start_index = 0
            # task_label = []
            # loss_list = []
            # outputs, aux_loss = model(data)
            # for index ,(task_name, task_num)in enumerate(task_dict.items()):        
            #     loss = torch.nn.functional.binary_cross_entropy_with_logits(outputs[task_name], labels[:,start_index:start_index+task_num])
            #     start_index+=task_num
            #     loss_list.append(loss)

            # with autocast(dtype=torch.float16):
            #     outputs, aux_loss = model(data)

            #     for task_name, task_num in task_dict.items():
            #         loss = F.binary_cross_entropy_with_logits(
            #             outputs[task_name],
            #             labels[:, start_index:start_index + task_num]
            #         )
            #         start_index += task_num
            #         loss_list.append(loss)


            #     sum_loss = AWL(loss_list)
            #     sum_loss = sum_loss + 0.5 * aux_loss

            outputs, aux_loss = model(data)

            # # If outputs is dict (MoE), collapse it
            # if isinstance(outputs, dict):
            #     outputs = torch.cat([v for v in outputs.values()], dim=1)

            # # reduce to single logit
            # outputs = outputs.mean(dim=1, keepdim=True)

            if isinstance(outputs, dict):
                outputs = outputs["aml"]

            loss = F.binary_cross_entropy_with_logits(outputs, labels)
            sum_loss = loss

            scaler.scale(sum_loss).backward()
            scaler.step(optim)
            scaler.update()

            # sum_loss = AWL(loss_list)
            # sum_loss = sum_loss+0.5*aux_loss
            # sum_loss.backward()
            # optim.step()
        
            global_step+=1
            loss_all["loss_sum"]+=sum_loss.item()
            loss_all["loss_count"]+=1 

            # global_step = 30000
            if global_step%100 == 0:
                mean_loss = loss_all["loss_sum"]/loss_all["loss_count"]
                if dist.get_rank() == 0 and args.wandb_report:
                    wandb.log({"loss":mean_loss})   
                    
            if global_step%10000 == 0:
            # if global_step%30 == 0:
                model_path_last = "./models/%s_%s_last.pt" %(histone,model_name)
                if args.save_model:
                    if dist.get_rank() == 0:
                        torch.save({
                            'model_state_dict': model.state_dict(),
                            'optimizer_state_dict': optim.state_dict()
                        },model_path_last)

                # if dist.get_rank() == 0:
                vali_loss, metrics_vali= evaluate_model(model, vali_loader,device)
                print(vali_loss)
                print(metrics_vali)


                torch.distributed.barrier()
        
                if dist.get_rank() == 0 and args.wandb_report:
                    wandb.log({"vali_loss": vali_loss})
                    wandb.log(metrics_vali)
        
                if best_loss> vali_loss:
                    best_loss=vali_loss
                    early_stop = 0
                    if args.save_model:
                        if dist.get_rank() == 0:
                            torch.save({
                                'model_state_dict': model.state_dict(),
                                'optimizer_state_dict': optim.state_dict()
                            },
                                model_path)
                        
                    test_loss, metrics_test = evaluate_model(model, test_loader, device)

                    if dist.get_rank() == 0:
                        test_result_dict = {k+"_test": v.item() for k,v in metrics_test.items()}
                        print(test_result_dict)

                        if args.wandb_report:
                            wandb.log({"test_loss": test_loss})
                            wandb.log(test_result_dict)
        
                else:
                    early_stop+=1
                model.train()

# trainer.train()
    if dist.get_rank() == 0 and args.wandb_report:
        wandb.finish()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Fine-tune model",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-w", "--wandb_report", type=bool,default=False, help="wandb or not")
    parser.add_argument("-r", "--reload", type=bool,default=False, help="reload or not")
    parser.add_argument("-d", "--wandbId", type=str, help="wandb or not")
    parser.add_argument('--freeze_layer', type=bool, default=False,
                        help='freeze_layer or not')
    parser.add_argument('--epochs', type=int, default=20, metavar='N',
                        help='number of epochs to train (default: 10)')
    parser.add_argument('--seed', type=int, default=1, metavar='S',
                        help='random seed (default: 1)')
    parser.add_argument('--save_model', type=bool, default=False,
                        help='For Saving the current Model')
    args = parser.parse_args()
    torch.manual_seed(args.seed)

    main(args)
